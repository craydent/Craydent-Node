...
export default function _getBrowserVersion(win: Window | Craydent, browser: string): number {
    try {
        let index = (win as Window).navigator.userAgent.indexOf(browser);
        if (!~index && (win as any)[`is${browser}`]()) {
        ...
        let endIndex: any = (win as Window).navigator.userAgent.indexOf(' ', index);
