...
import { MongoReducer, MongoMapReduceOptions } from '../models/Arrays';
...
export default function mapReduce<T, TResult>(objs: T[], map: (this: T) => void, reduce: MongoReducer<TResult>, options?: MongoMapReduceOptions<TResult>): TResult[] {
    /*|{
    ...
    try {
        options = options || {};
        let obj: any = {}, results: any = where(objs, options.query, null as any, options.limit), rtnArr: any[] = [], final: any = options.finalize;
        if (options.sort) {
            results = sortBy(results, options.sort);
        }
        on(map, 'emit', function (key: any, value: any) {
            obj[key] = obj[key] || [];
            obj[key].push(value);
        });
        ...
        if (isString(options.out)) {
            (global as any)[options.out as string] = duplicate(rtnArr, true);
