import $c from '../../transformedMajor/template/global';
...
\${end foreach}`, { TASK: { subtasks: [{ sub_complete: true }, { sub_complete: false }] } }).trim())
    ...
    it('fillTemplate - script/try', function () {
        ...
        expect(fillTemplate("<div>${try}echo('asdf');var a = null; a.foo;${catch (e)}echo(e);${finally}echo('finallyy')${end try}<div>", { a: 'monday', b: 'tuesday' })).toBe("<div>asdfError: TypeError: Cannot read property 'foo' of nullfinallyy<div>");
    });
