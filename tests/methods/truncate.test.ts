...
let _truncate = (...args: any[]) => { args[args.length - 1](); };
describe('truncate', () => {
    beforeEach(() => {
        _truncate = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _truncate = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _truncate = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
