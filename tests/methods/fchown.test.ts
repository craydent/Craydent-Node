...
let _fchown = (...args: any[]) => { args[args.length - 1](); };
describe('fchown', () => {
    beforeEach(() => {
        _fchown = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _fchown = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _fchown = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
