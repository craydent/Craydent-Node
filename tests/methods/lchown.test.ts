...
let _lchown = (...args: any[]) => { args[args.length - 1](); };
describe('lchown', () => {
    beforeEach(() => {
        _lchown = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _lchown = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _lchown = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
