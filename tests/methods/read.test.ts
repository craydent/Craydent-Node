...
let _read = (...args: any[]) => { args[args.length - 1](); };
describe('read', () => {
    beforeEach(() => {
        _read = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _read = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _read = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
