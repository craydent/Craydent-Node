...
jest.mock('fs', () => {
    return {
        "readFile": (...args) => fs.apply(this, args)
    }
});
let __setPath = () => { }
let fs = () => { }
describe('createServer', () => {
    ...
    it('should handle favicon.ico with 200', () => {
        fs = jest.fn().mockImplementationOnce((name, f) => f(null, 'content'));
        const req = {
            url: '/favicon.ico',
        ...
        createServer({ createServer: (func) => { return func(req, res), {} as any; }, favicon: 'fav' });
    ...
    it('should handle favicon.ico with 500', () => {
        fs = jest.fn().mockImplementationOnce((name, f) => { throw '1' });
        const req = {
            url: '/favicon.ico',
            headers: { host: 'www.example.com' },
            connection: {}
        ...
        const http = createServer({ createServer: (func) => { return func(req, res), {} as any; }, favicon: 'fav' });
    ...
    it('should handle post request', () => {
        fs = jest.fn().mockImplementationOnce((name, f) => { throw '' });
        const req = {
            url: '/path',
            headers: { host: 'www.example.com', 'content-type': '/x-www-form-urlencoded' },
            connection: {},
            method: 'post',
            on: (event, func) => func('post=1')
        };
        const res = { writeHead: jest.fn(), end: jest.fn() };
        let callback, cb = jest.fn();
        const http = createServer({ createServer: (func) => { callback = func; return {} as any; }, favicon: 'fav' });
    ...
    it('should handle multiple listener types', async () => {
        ...
        const res = { writeHead: jest.fn(), end: jest.fn() };
        let callback, cbs = [
            async function (req, res, params, next) {
                return await next();
            },
        ...
        const http = createServer({
            createServer: (func) => { callback = func; return {} as any; },
            favicon: 'fav',
            callback: async (req, res, value) => { cb(req, res, value); }
        });
    ...
    it('should handle generator listener', async () => {
        ...
        const http = createServer({
            createServer: (func) => { callback = func; return {} as any; },
            favicon: 'fav',
            callback: function* (req, res, value) { cb(req, res, value); }
        });
        http.routes = [{ path: '/*', callback: cbs, method: 'get' }];
        await callback(req, res);
        expect(res.end).toHaveBeenCalledWith(JSON.stringify({ f: 1 }));
    ...
    it('should invalidate parameters', () => {
        ...
        const req = {
            ...
            connection: {},
            method: 'post',
            on: (event, func) => func('post=1&dtInvalid=abc&postNumber=def&email2=ghi')
        };
        const res = { writeHead: jest.fn(), end: jest.fn() };
        let callback, cb = jest.fn();
        const http = createServer({ createServer: (func) => { callback = func; return {} as any; }, favicon: 'fav' });
