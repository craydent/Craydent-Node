...
let _access = (...args: any[]) => { args[args.length - 1](); };
describe('access', () => {
    beforeEach(() => {
        _access = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _access = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _access = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
