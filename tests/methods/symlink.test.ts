...
let _symlink = (...args: any[]) => { args[args.length - 1](); };
describe('symlink', () => {
    beforeEach(() => {
        _symlink = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _symlink = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _symlink = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
