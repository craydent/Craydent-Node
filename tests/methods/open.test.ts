...
let _open = (...args: any[]) => { args[args.length - 1](); };
describe('open', () => {
    beforeEach(() => {
        _open = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _open = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _open = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
