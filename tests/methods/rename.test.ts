...
let _rename = (...args: any[]) => { args[args.length - 1](); };
describe('rename', () => {
    beforeEach(() => {
        _rename = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _rename = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _rename = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
