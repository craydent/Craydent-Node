...
let _fchmod = (...args: any[]) => { args[args.length - 1](); };
describe('fchmod', () => {
    beforeEach(() => {
        _fchmod = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _fchmod = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _fchmod = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
