...
let _chown = (...args) => { args[args.length - 1](); };
describe('chown', () => {
    beforeEach(() => {
        _chown = (...args) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _chown = jest.fn().mockImplementationOnce((...args) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _chown = jest.fn().mockImplementationOnce((...args) => { args[args.length - 1]({}); });
