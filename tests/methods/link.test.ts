...
let _link = (...args: any[]) => { args[args.length - 1](); };
describe('link', () => {
    beforeEach(() => {
        _link = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _link = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _link = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
