...
let _appendFile = (...args: any[]) => { args[args.length - 1](); };
describe('appendFile', () => {
    beforeEach(() => {
        _appendFile = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _appendFile = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _appendFile = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
