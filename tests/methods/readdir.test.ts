...
let _readdir = (...args: any[]) => { args[args.length - 1](); };
describe('readdir', () => {
    beforeEach(() => {
        _readdir = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _readdir = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _readdir = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
