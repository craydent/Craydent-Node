...
let _readlink = (...args: any[]) => { args[args.length - 1](); };
describe('readlink', () => {
    beforeEach(() => {
        _readlink = (...args: any[]) => { args[args.length - 1](); };
    })
    it('should return null when there are no errors', async () => {
        _readlink = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1](null); });
    ...
    it('should return error when there are errors', async () => {
        _readlink = jest.fn().mockImplementationOnce((...args: any[]) => { args[args.length - 1]({}); });
