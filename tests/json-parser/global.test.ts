...
describe('No Conflict Global methods', function () {
    it('JSON.parseAdvanced', function () {
        ...
        expect(JSON.stringify(parseAdvanced({ "routes": { "Function.${bb.b}": "function(${domain}){}", "${domain}": "${bb.b}" } }, null, { domain: "property", bb: { b: "baby" } }))).toEqual(JSON.stringify({ routes: { baby: function (property: any) { }, property: "baby" } }));
