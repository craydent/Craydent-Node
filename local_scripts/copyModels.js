...
async function start(prefix) {
    console.log(CONSOLE_COLORS.GREEN, 'copying models');
    var root = require.resolve('../package.json').replace('/package.json', '');
    await _cli_exec(`cp -R  ${root}/modules/models ${root}/compiled/models`);
    console.log(CONSOLE_COLORS.GREEN, 'finished copying models')
